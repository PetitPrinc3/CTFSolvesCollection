...
def encode_sample(sample):
    if sample == ".":
        res = np.full((1,24), (1+1j), dtype = np.complex64)

    elif sample == "_":
        res = np.full((1,72), (0j), dtype = np.complex64)


    elif sample == "-":
        res = np.full((1,120), (1+1j), dtype = np.complex64)
    elif sample == " ":
        res = np.full((1,120*3+72), (0j), dtype = np.complex64)
...
def am_encode(s):
    enc = np.array([], dtype = np.complex64)
    for l in range(len(s)-1):

        res = encode_sample(s[l])
        res1 = np.full((1,24), (0j), dtype = np.complex64)
        res = np.append(res, res1)
        enc = np.append(enc, res)

    enc = np.append(enc, encode_sample(s[-1]))
    return enc
...
def main():
    ...
    try:
        ...
        signal = np.frombuffer(base64.b64decode(a), dtype = np.complex64)
